...
function [errDt,sol]=paraoptNumLotkaEq()
    %%%%% Espace data
    T=1;
    sigma=2;%8;
    c=0.2;
alpha=1e-2;
    ...
    %%% Linear system %%%%%%%%%%%%%%%%%%%%
     ...
     N=2.^kk;
     itermax=10;
     errDt=zeros(10,itermax);
     %raidus=zeros(10,1);
     
     for k=1:limit-1
     %N=5;
	     Dt=DT/N(k);
	     MDt=matrix(calL,B,r,A,b,s,L,N(k),Dt,alpha,d);
	     M=itermatrix(MDt,Mdt);
	     [~,err]=solveriterative(M,MDt\bf,sol,itermax);
	     iter=iter+1;
	     errDt(iter,:)=err;
	    % raidus(iter)=spectralradius(M);
	...
	figure(022)
	for n=1:2:limit-1
	 	semilogy(1:length(errDt(n,:)),errDt(n,:)','o-');
	 	hold on
	end
	Fontsize=10;
	 xlabel('\# Iters','interpreter','latex','FontSize',Fontsize);
     ylabel('error','interpreter','latex','FontSize',Fontsize);
     %legend({'$$\Delta t=\Delta T$$','$$\Delta t=\Delta T/6$$',...
     %    '$$\Delta t=\Delta T/10$$','$$\Delta t=\Delta T/14$$'},'interpreter','latex','FontWeight','bold','Location','Southeast','FontSize',Fontsize);
         grid on
	hold off
  %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
  ...
  N=2.^k;
    %%% Linear system %%%%%%%%%%%%%%%%%%%%
     ...
     %N=5;
	     ...
	     EstimateBound(i)=max(calL(1)*(Dt-dt)*(0.5+...
    (calL(1)*Dt/2+1)*exp(2*calL(1)*DT)),calL(2)*(Dt-dt)) ; 
	...
	figure(028)
	loglog(DT./N,raidus,'o-',DT./N,EstimateBound,'d-');
	hold on
